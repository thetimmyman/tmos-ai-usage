// shell/plugins/tmos.usage/BarWidget.qml — the one surface.
//
// Bar-widget entry point (manifest.json entryPoints.barWidget). Built on `qs.Ui.Panel`, the base
// first-party and third-party popout widgets use: it owns the IPC-backed open/close lifecycle, so
// `omarchy-shell ipc call tmos.usage toggle` works for free and a keybinding needs no code here.
//
// S1-C finding that still governs this file (see ../tmos.intent/BarWidget.qml): the bar injects
// `bar`/`moduleName`/`settings`, never `shell`/`manifest`, and only a registered `qs.Ui` click
// target receives clicks — hence WidgetButton rather than a bare MouseArea.
//
// Every colour, size and font here is a qs.Commons token (gate N3): the widget renders as whatever
// theme is active, and TMOS ships no palette of its own. Tone names come from Model.js so the
// arithmetic stays out of the view.
//
// THE SHAPE OF THIS SURFACE (Tim's requirement, and it is not the donor's shape):
//
//   * Every subscription is on the first page. There is no provider switcher and no tab to press:
//     all five rows are always drawn, in the same order, so they can be compared down the page.
//   * The panel is sized so that all of them fit without scrolling. Only an expansion can make the
//     body taller than the panel, and that one scrolls.
//   * Clicking a row (or j/k + Enter) opens that row's depth underneath it — limits with reset
//     countdowns, the prepaid ledger, tokens by day, tokens by model. Depth is opt-in and one row
//     at a time, so opening one never hides another subscription.
//
// No network, no credentials, no endpoints in this process — UsageSource.qml reads a file that
// tools/usage_collector.py wrote.
import QtQuick
...
Ui.Panel {
    ...
    onProvidersChanged: {
        rotateIndex = 0;
        if (expandedId !== "" && !root.hasProvider(expandedId)) expandedId = "";
    }
    ...
    // j/k walk the list, opening each row as it goes, so the keyboard can read the panel without
    // a pointer. There is deliberately no left/right action: nothing gets switched away from.
    ...
    function toggleCurrent() {
        ...
        else root.toggleExpanded(root.expandedId);
    }
                ...
                Column {
                    ...
                    Item {
                        ...
                        Text {
                            ...
                            anchors.verticalCenter: parent.verticalCenter
                            textFormat: Text.PlainText
                            text: (source.refreshing ? "reading…" : Model.ageText(source.observedAt, root.nowMs)) + " · R refresh"
                                
                            color: root.stale && !source.refreshing ? Color.accent : root.dim
                            font.family: root.fontFamily
                    ...
                    Text {
                        width: parent.width
                        visible: source.loadError !== "" || source.refreshError !== ""
                        textFormat: Text.PlainText
                        text: source.refreshError !== "" ? source.refreshError : source.loadError
                        color: Color.urgent
                        font.family: root.fontFamily
                    ...
                    Ui.Button {
                        width: parent.width
                        bordered: true
                        text: source.refreshing ? "Refreshing…" : "Refresh now  (R)"
                        enabled: !source.refreshing
                        foreground: root.foreground
                        fontFamily: root.fontFamily
                    ...
                    Text {
                        width: parent.width
                        textFormat: Text.PlainText
                        text: "Read from " + source.cachePath + " · no network in this widget"
                        color: root.dim
                        font.family: root.fontFamily
                        font.pixelSize: Style.font.caption
                        wrapMode: Text.WordWrap
